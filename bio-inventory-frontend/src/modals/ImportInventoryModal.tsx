...
import { processImportFile, validateInventoryRow, ImportResult } from '../utils/excelImport.ts';
...
const ImportInventoryModal: React.FC<ImportInventoryModalProps> = ({ isOpen, onClose, onSuccess, token }) => {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const response = await fetch(buildApiUrl(API_ENDPOINTS.ITEMS_BATCH_CREATE), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Token ${token}`
        },
        body: JSON.stringify({ items: importResult.data })
      });

      if (response.ok) {
        const result = await response.json();
        notification.success(`Successfully imported ${result.created_count || importResult.validRows} items`);
        onSuccess();
        handleClose();
      } else {
  ...
  const downloadTemplate = () => {
    const template = [
      ['name', 'quantity', 'unit', 'location', 'vendor', 'item_type', 'unit_price', 'minimum_quantity', 'expiration_date', 'notes'],
      ['Sample Item', '10', 'pcs', 'Lab A', 'Vendor Inc', 'Equipment', '25.99', '5', '2024-12-31', 'Sample notes']
    ];
    ...
    <div className="modal-overlay">
      <div className="modal-container max-w-4xl">
        <div className="modal-header">
          <h2 className="modal-title">Import Inventory Data</h2>
          ...
          {!showPreview ? (
            ...
            >
              {importing ? (
                ...
                `Import ${importResult?.validRows || 0} Items`
              )}
            </button>
          )}
        </div>
