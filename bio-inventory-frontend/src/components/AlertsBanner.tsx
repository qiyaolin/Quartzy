    ...
    useEffect(() => {
        if (token) {
            ...
            .then(data => {
                ...
                const safeData = {
                    expired: data?.expired || { count: 0, items: [] },
                    expiring_soon: data?.expiring_soon || { count: 0, items: [] },
                    low_stock: data?.low_stock || { count: 0, items: [] }
                };
                setAlerts(safeData);
            })
            .catch(error => {
                ...
                setAlerts({ expired: { count: 0 }, expiring_soon: { count: 0 }, low_stock: { count: 0 } });
            });
        }
    }, [token]);

    const totalAlerts = (alerts?.expired?.count || 0) + (alerts?.expiring_soon?.count || 0) + (alerts?.low_stock?.count || 0);
            ...
            <div className="max-w-7xl mx-auto px-4 py-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <span className="text-sm font-medium text-warning-800">
                            ...
                            {(alerts?.expired?.count || 0) > 0 && ` ${alerts.expired.count} expired`}
                            {(alerts?.expiring_soon?.count || 0) > 0 && ` ${alerts.expiring_soon.count} expiring soon`}
                            {(alerts?.low_stock?.count || 0) > 0 && ` ${alerts.low_stock.count} low stock`}
                        </span>
                    </div>
                ...
                {isExpanded && (
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        {(alerts?.expired?.count || 0) > 0 && (
                            <div className="bg-danger-50 p-3 rounded-lg border border-danger-200">
                                <h4 className="text-sm font-semibold text-danger-800 mb-2">Expired Items ({alerts?.expired?.count || 0})</h4>
                        ...
                        {(alerts?.expiring_soon?.count || 0) > 0 && (
                            <div className="bg-warning-50 p-3 rounded-lg border border-warning-200">
                                <h4 className="text-sm font-semibold text-warning-800 mb-2">Expiring Soon ({alerts?.expiring_soon?.count || 0})</h4>
                        ...
                        {(alerts?.low_stock?.count || 0) > 0 && (
                            <div className="bg-primary-50 p-3 rounded-lg border border-primary-200">
                                <h4 className="text-sm font-semibold text-primary-800 mb-2">Low Stock ({alerts?.low_stock?.count || 0})</h4>
